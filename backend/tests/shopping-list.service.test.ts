...
Deno.test('ShoppingListService - create and retrieve item', async () => {
  ...
  const fetched = await shoppingListBackendService.getItemById(created.id);
  assertEquals(fetched?.id, created.id);

  // Clean up
  await shoppingListBackendService.deleteItem(created.id);
  db.close();
});
...
Deno.test('ShoppingListService - bulk insert and delete checked', async () => {
  ...
  const deletedCount = await shoppingListBackendService.deleteCheckedItems();
  assertEquals(deletedCount, 1);

  // Clean up remaining item
  for (const item of items) {
    await shoppingListBackendService.deleteItem(item.id);
  }
  db.close();
});
